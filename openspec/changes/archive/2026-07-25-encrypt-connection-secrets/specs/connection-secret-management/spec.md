...
connector's declared parameter metadata (parameters of type `ProtectedText`), not from a value
...
- **THEN** values submitted for that parameter key are treated as secret and encrypted at rest

#### Scenario: A non-ProtectedText parameter is treated as non-secret
- **WHEN** a connector declares a parameter with a type other than `ProtectedText`
- **THEN** values submitted for that parameter key are stored as plaintext
