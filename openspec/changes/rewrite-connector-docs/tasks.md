...
- [ ] 1.1 Re-read `TheGrid.Connectors/ConnectorBase.cs`, `Attributes/ConnectorAttribute.cs`,
...
- [ ] 2.1 Replace the "Getting started" section's base-class/attribute names
      (`QueryRunnerBase`/`QueryRunnerAttribute`) with `ConnectorBase`/`ConnectorAttribute`.
- [ ] 2.2 Replace the invented `MyDatabaseRunner` worked example with one built around
...
- [ ] 2.3 Rewrite the `GetDataAsync` section to document the actual signature —
...
- [ ] 2.4 Rewrite the `ConnectorParameterAttribute` table to match real properties (`Name`,
      ...
      `Boolean`).
- [ ] 2.5 Add an explicit note (current-behavior, not aspirational) that `ConnectorParameters` dictionary
...
- [ ] 2.6 Rewrite the "Adding database schema discovery support" section around the real
...
- [ ] 2.7 Add a section documenting `IConnectionTest.TestConnectionAsync() : Task<bool>` and
...
- [ ] 2.8 Remove any remaining references to `QueryRunner`/`QueryRunnerBase`/`RunQueryAsync`/
...
- [ ] 3.1 Grep `docs/Creating-Connectors.md` for `QueryRunner` (case-insensitive) — must return no
      matches.
- [ ] 3.2 Manually diff every code excerpt in the doc against its source file
      (`PostgreSqlConnector.cs`) line-by-line to confirm it's a faithful excerpt, not paraphrased or
      stale.
- [ ] 3.3 Confirm the doc's attribute/type/method names all exist in the current codebase (spot-check
